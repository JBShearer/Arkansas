...
function updateChips() {{
  ...
  types.forEach(t => {{
    ...
    if (!el) return;
    const n = ALL_CAPS.filter(c => c.is_leaf && c.capability_type === t).length;
    el.textContent = n ? ' ' + n : '';
  }});
}}

function updateResultMeta() {{
  const total = ALL_CAPS.filter(c => c.is_leaf && !excludedProducts.has(c.product)).length;
  const filtered = getFilteredCaps().filter(c => c.is_leaf).length;
