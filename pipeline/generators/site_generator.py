...
function ucCount(c) {{
  return (c.use_cases && c.use_cases.length > 0) ? c.use_cases.length : 1;
}}
...
function renderTypeCards(caps) {{
  const types = {{}};
  caps.filter(c => c.is_leaf).forEach(c => {{
    const t = c.capability_type;
    if (t) types[t] = (types[t] || 0) + ((c.use_cases && c.use_cases.length) ? c.use_cases.length : 1);
  }});
...
const ALL_PRODUCTS = (function() {{
  ...
  ALL_CAPS.forEach(c => {{
    if (!c.is_leaf) return;
    counts[c.product] = (counts[c.product] || 0) + ((c.use_cases && c.use_cases.length) ? c.use_cases.length : 1);
  }});
...
function updateChips() {{
  ...
  types.forEach(t => {{
    ...
    if (!el) return;
    const n = ALL_CAPS.filter(c => c.is_leaf && c.capability_type === t).reduce((s,c) => s + ((c.use_cases && c.use_cases.length) ? c.use_cases.length : 1), 0);
    el.textContent = n ? ' ' + n : '';
  }});
}}

function updateResultMeta() {{
  const total = ALL_CAPS.filter(c => c.is_leaf && !excludedProducts.has(c.product)).reduce((s,c) => s + ((c.use_cases && c.use_cases.length) ? c.use_cases.length : 1), 0);
  const filtered = getFilteredCaps().filter(c => c.is_leaf).reduce((s,c) => s + ((c.use_cases && c.use_cases.length) ? c.use_cases.length : 1), 0);
